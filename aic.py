...
print (conf.WORD_LIST_FILE_PATH)
...
def log(string, logtype = 0, rewriteable = False):
	...
	if rewriteable:
		print("[" + colored(str(dt.today().strftime("%Y/%m/%d %H:%M:%S")).split(".")[0], "blue") + "] [" + type_indicator + "] " + string, end="\r")
	else:
		print("[" + colored(str(dt.today().strftime("%Y/%m/%d %H:%M:%S")).split(".")[0], "blue") + "] [" + type_indicator + "] " + string)
...
class Air_gui(Gtk.ApplicationWindow):
	def __init__(self, app):
		...
		headerbar.props.title = "WIFI Penetration Test Tool"
	...
	def on_button_toggled_airmon(self, ):
	...
	def on_button_clicked_aircrack(self):
	...
	def on_button_clicked_scan(self):
...
class Scan(Gtk.ApplicationWindow):
	def __init__(self, app):
		...
		for index, network in enumerate(networks):
			...
			label = Gtk.Label()
			label.set_markup("<big>BSSID: </big>{}\n<big>Channel: </big>{}\n<big>Signal Strength: </big>{}%\n<big>Encryption: </big>{} \n<big>scan time: </big>{}".format(network[0], network[2], network[3], network[4],dt.now().strftime("%Y/%m/%d %H:%M:%S")))
	...
	def scan_networks(self):
		...
		for network in networks:
			db.create_record(network[0], network[2], network[3], network[4])
		return networks
...
#WIFI Penetration Test Tool window
class Aircrack_ng(Gtk.ApplicationWindow):
	...
	def __init__(self, app):
		...
		hb.props.title = "WIFI Penetration Test Tool"
		...
		self.chooseWordlistFilePath = Gtk.Button(label = "   Choose wordlist    ") # 
...
class Aircrack_gui(Gtk.Application):
	...
	def do_command_line(self, command_line):
		...
		global DO_NOT_LOG
		DO_NOT_LOG = True if "nolog" in options and options["nolog"] is True else False 	  #
		global DO_NOT_CLEAN
		DO_NOT_CLEAN = True if "noclean" in options and options["noclean"] is True else False #
